...
public class MouseInput {
    ...
    public MouseInput(long windowHandle) {
        ...
        // set cursor ini untuk setiap kali pencet di layar dia mendeteksi koordinatnya
        glfwSetCursorPosCallback(windowHandle, (handle, xpos, ypos) -> {
            currentPos.x = (float) xpos;
            currentPos.y = (float) ypos;
        });
        // setiap kali masuk ke window ini yg akan dipanggil
        glfwSetCursorEnterCallback(windowHandle, (handle, entered) -> inWindow = entered);
        glfwSetScrollCallback(windowHandle, (handle, xoffset, yoffset)->{
            scroll.x = (float)xoffset;
            scroll.y = (float)yoffset;
        });
        // ini untuk mendeteksi klik yg mana (klik kanan, klik kiri, release dll)
